...
``MONGO_USE_GREENLETS``      Set to ``True`` to enable will ensure that the current greenlet uses the same socket for all
                             operations until :meth:`end_request()` Default: ``False``.
